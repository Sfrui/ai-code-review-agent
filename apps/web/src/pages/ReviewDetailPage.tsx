...
import { ArrowLeft, Play, RotateCcw, FileCode, AlertCircle, GitCompare, Filter } from 'lucide-react';
...
import type { ReviewIssue } from '@ai-review/shared';

// ============================================================
// ReviewDetailPage — 审查详情页（含问题筛选功能）
// ============================================================
...
const severityOptions = [
  ...
  { value: 'error' as const, label: '严重', color: 'text-red-600 bg-red-50 dark:bg-red-900/20' },
  { value: 'warning' as const, label: '警告', color: 'text-amber-600 bg-amber-50 dark:bg-amber-900/20' },
  { value: 'info' as const, label: '建议', color: 'text-blue-600 bg-blue-50 dark:bg-blue-900/20' },
] as const;
...
export function ReviewDetailPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* 审查结果摘要 */}
      {hasResult && task.reviewResult && (
        <ReviewSummary reviewResult={task.reviewResult} />
      )}
        ...
        <div className="border-b border-surface-200 dark:border-surface-700">
          <div className="flex gap-1">
            {[
              ...
              { key: 'issues' as const, label: '问题列表', icon: AlertCircle },
        ...
        <div className="animate-fade-in">
          ...
          {activeTab === 'issues' && (
            <div className="space-y-4">
              {/* 问题筛选器 */}
              {task.reviewResult.issues.length > 0 && (
              ...
              {filteredIssues.length === 0 ? (
                <EmptyState
                  icon={AlertCircle}
                  title={severityFilter === 'all' ? '没有发现问题' : `没有${severityFilter === 'error' ? '严重' : severityFilter === 'warning' ? '警告' : '建议'}级别的问题`}
                  description={
                    severityFilter === 'all'
                      ? '代码质量很好，继续保持！'
                      : '尝试切换筛选条件查看其他问题'
                ...
                filteredIssues.map((issue, index) => (
                  <IssueCard key={`${issue.line}-${index}`} issue={issue} index={index} />
                ))
              )}
            </div>
          )}
          ...
          {activeTab === 'diff' && (
            <DiffView
              ...
              newCode={
                // 生成修改后的代码：按行替换 fixedCode
                task.reviewResult.issues
